...
@Model.register("seq2labels")  # inherits from AllenNLP model class
class Seq2Labels(Model):
    """
    This ``Seq2Labels`` simply encodes a sequence of text with a stacked ``Seq2SeqEncoder``, then
    predicts a tag (or couple of tags) for each token in the sequence.
    ...
    calculate_span_f1 : ``bool``, optional (default=``None``)
        ...
        If ``None`` and label_encoding is specified, this is set to ``True``.
        If ``None`` and label_encoding is not specified, it defaults to ``False``.
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        self.incorr_index = self.vocab.get_token_index("INCORRECT", namespace=detect_namespace)
    ...
    @overrides  # @ = decorator -> override method from parent class
