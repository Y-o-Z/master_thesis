...
from transformers import AutoTokenizer  # general purpose architectures for

from g_utils.helpers import START_TOKEN

logger = logging.getLogger(__name__)

# This is the default list of tokens that should not be lowercased.
_NEVER_LOWERCASE = ['[UNK]', '[SEP]', '[PAD]', '[CLS]', '[MASK]']  # classification
...
class WordpieceIndexer(TokenIndexer[int]):
    ...
    def __init__(self,
                 ...
                 bpe_ranks: Dict,
                 byte_encoder: Dict,
                 wordpiece_tokenizer: Callable[[str], List[str]],  # in, out
...
class PretrainedBertIndexer(WordpieceIndexer):  # [Y]
    # pylint: disable=line-too-long  # py + lint → c
    """
