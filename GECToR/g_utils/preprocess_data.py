"""
Copyright (c) 2020 Grammarly - GECToR (https://github.com/grammarly/gector)
"""
...
from helpers import write_lines, read_parallel_lines, encode_verb_form, \
    apply_reverse_transformation, SEQ_DELIMITERS, START_TOKEN
...
def is_sent_ok(sent, delimiters=SEQ_DELIMITERS):
    for del_val in delimiters.values():
...
def add_labels_to_the_tokens(source_tokens, labels, delimiters=SEQ_DELIMITERS):
    ...
    for token, label_list in zip(source_tokens_with_start, labels):
        all_tags = delimiters['operations'].join(label_list)
        comb_record = token + delimiters['labels'] + all_tags
        tokens_with_all_tags.append(comb_record)
    return delimiters['tokens'].join(tokens_with_all_tags)
...
def convert_tagged_line(line, delimiters=SEQ_DELIMITERS):
    label_del = delimiters['labels']
    source_tokens = [x.split(label_del)[0]
                     for x in line.split(delimiters['tokens'])][1:]
    labels = [x.split(label_del)[1].split(delimiters['operations'])
              for x in line.split(delimiters['tokens'])]
